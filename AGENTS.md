...
主機同時搭載兩張無線網路卡，各自肩負嚴格區隔之任務（支援跨機器動態偵測，如 Ubuntu 或 Kali Linux）：
1. **主力對外連線網卡（如 Ubuntu `wlxd03745e1db0d` 或 Kali `wlan0`）**：
   * **角色**：專職連接外部網際網路，並透過 `iptables` 提供 NAT IP 偽裝（Masquerade）與封包轉送。
   * **路由度量**：Metric 100（或系統預設閘道優先權）。
   * ⚠️ **【鐵律】絕不可中斷、關閉（down）、重置或更動此網路卡的連線、IP 與路由設定**，否則將導致連線中斷（包含 SSH 斷線）。
2. **ALFA 診斷測試網卡（如 Ubuntu `wlx00c0cabb0b45` 或 Kali `wlan1`）**：
   * **角色**：專職作為診斷基地台（Diagnostic AP）、空中封包側錄、周遭熱點 AP 掃描或 Client 備援模式。
